...
for d in ${DIST}/*.html ; do
    echo "$d"
    echo `basename $d`
    gzip -9 -c ${d} | aws s3 cp - s3://blog.nikpappas.com/`basename $d` --acl "public-read" --content-encoding "gzip" --content-type "text/html" --profile "papakos_1"
done
...
# aws s3 cp ./out/videos/gravity-simulator.mkv s3://blog.nikpappas.com/videos/gravity-simulator.mkv --acl "public-read" --content-type "video/x-matroska" --profile "papakos_1"
